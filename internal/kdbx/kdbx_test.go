...
func getProjectPath(relPath string) string {
	...
	// Путь к директории пакета kdbx_test
	kdbxDir := path.Dir(filename)
	// Поднимаемся на два уровня вверх (internal/kdbx_test -> /), чтобы получить корень проекта
...
func skipIfTestFileNotExist(t *testing.T, filePath string) {
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		t.Skipf("Тестовый файл не найден по пути %s, пропускаем тест", filePath)
	}
}

func TestOpenFile(t *testing.T) {
	testFilePath := getProjectPath("example/test.kdbx")
	// Проверяем доступность тестового файла
	skipIfTestFileNotExist(t, testFilePath)

	// Тест 1: Успешное открытие файла с правильным паролем
	t.Run("Success_With_Correct_Password", func(t *testing.T) {
		db, err := kdbx.OpenFile(testFilePath, testPassword)
	...
	t.Run("Error_With_Incorrect_Password", func(t *testing.T) {
		db, err := kdbx.OpenFile(testFilePath, invalidPassword)
	...
	t.Run("Error_With_Nonexistent_File", func(t *testing.T) {
		...
		db, err := kdbx.OpenFile(nonExistentPath, testPassword)
...
func TestGetAllEntries(t *testing.T) {
	...
	t.Run("Handle_Nil_Database", func(t *testing.T) {
		entries := kdbx.GetAllEntries(nil)
...
func TestCreateDatabase(t *testing.T) {
	...
	t.Run("Create_New_Database", func(t *testing.T) {
		...
		_, err := kdbx.CreateDatabase(tempFile, newPassword)
